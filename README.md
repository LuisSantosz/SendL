...
Importe o CNAB (ou títulos em Excel), depois clique em Buscar documentos no Gmail. Todas as consultas e páginas são processadas automaticamente a partir dos títulos e CPF/CNPJ da remessa. Interromper busca mantém os vínculos já concluídos. Não é preciso selecionar cliente ou tipo. O sistema cruza títulos completos ou números de NF com a remessa e procura CPF/CNPJ nas seções de pagador/sacado/destinatário do PDF. Não usa nome parecido, remetente ou CNPJ do emitente para escolher o cliente. Não converte o prefixo de títulos bancários em número de NF.

PDFs ambíguos, ilegíveis, digitalizados sem texto ou acima de 20 páginas ficam pendentes com motivo. Clientes presentes na remessa são cadastrados sem e-mail quando necessário; complete o e-mail antes de enviar. Buscar novamente ignora anexos já coletados, inclusive enviados. A associação não dispara envio. A leitura do PDF ocorre no servidor, com limite de tempo e memória. Instale as novas dependências com `npm install` e use Node 22.16 ou superior.


### Correções 2.2.1

CPF/CNPJ são comparados com o preenchimento de 14 posições usado pelo CNAB, preservando os valores originais. A fila reconhece o e-mail já cadastrado mesmo sem os zeros bancários. Cadastros equivalentes com e-mails diferentes ficam sinalizados para revisão. A busca exige remessa pendente, usa consultas direcionadas por título e documento, percorre todas as páginas e não apresenta botões de vinculação manual nos resultados. Sacador/Avalista não é interpretado como Sacado. O CPF/CNPJ isolado não basta para coletar todos os documentos antigos de um cliente: é necessário título/nota correspondente ou pagador + valor + vencimento. A consulta depende da indexação do Gmail; PDFs sem texto ou layouts não reconhecidos continuam sinalizados, sem envio automático.
